...
const payloadDefault = {
    ...
    input_face:
        'https://storage.googleapis.com/dara-c1b52.appspot.com/daras_ai/media/e3d155ea-0a4b-11ef-aee5-02420a00012b/LongMuteClip.mp4',
};

const buildPayload = (mode: string, text: string) => {
    return {
        ...payloadDefault,
        ...(mode === 'ELEVEN_LABS'
            ? {
...
export async function gooeyAPI(text: string): Promise<{ video?: unknown; error?: string | Response }> {
    const mode =
        process.env['USE_ELEVEN_LABS'] === 'true' && process.env['ELEVEN_LABS_API_KEY'] ? 'ELEVEN_LABS' : 'DEFAULT';
